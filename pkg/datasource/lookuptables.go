...
func coerceLookupTableFilterTypes(types map[string]schemas.LookupTableColumnType, filter *schemas.LookupTableRowFilter) *schemas.LookupTableRowFilter {
	...
	for _, group := range filter.ConditionGroups {
		...
		// value list stays nil rather than becoming an empty one. The historian reads an absent
		// list and an empty one alike, and the pruning above means every operator that does read
		// values arrives here holding some.
		if group.Values != nil {
...
func unfinishedLookupTableCondition(group schemas.LookupTableRowConditionGroup) bool {
	...
	case schemas.LookupTableRowOperatorIsNull, schemas.LookupTableRowOperatorIsNotNull:
		return false
	default:
