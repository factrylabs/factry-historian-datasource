...
holding a number never matches one, and they compare against a single value: a variable that
resolves to several is read as the first of them rather than as the loosest bound among them. `Columns` is a projection, empty meaning every column of
