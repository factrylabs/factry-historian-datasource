...
describe('DataSource.applyTemplateVariables lookup tables', () => {
  ...
  // An ordering compares against one value, and its box in the editor holds one, but a
  // variable in it can resolve to several. The historian reads every operator's values as
  // "any one of", so sending them all would turn "> 10" into the loosest of the bounds and
  // quietly return more rows than the panel asks for.
  it.each(['>', '>=', '<', '<='] as const)('keeps a multi-value variable to one value under %s', (operator) => {
    const ds = makeDataSource(makeTemplateSrv({ threshold: ['10', '20', '30'] }))
    const result = ds.applyTemplateVariables(
      makeLookupTableQuery({ Filter: filterOn({ Column: 'setpoint', Operator: operator, Values: ['$threshold'] }) }),
      {}
    )
    expect((result.query as LookupTableQuery).Filter).toEqual(
      filterOn({ Column: 'setpoint', Operator: operator, Values: ['10'] })
    )
  })

  it('expands a multi-value variable under an operator reading a set', () => {
    const ds = makeDataSource(makeTemplateSrv({ lines: ['L1', 'L2'] }))
    const result = ds.applyTemplateVariables(
      makeLookupTableQuery({ Filter: filterOn({ Column: 'line', Operator: 'NOT IN', Values: ['$lines'] }) }),
      {}
    )
    expect((result.query as LookupTableQuery).Filter).toEqual(
      filterOn({ Column: 'line', Operator: 'NOT IN', Values: ['L1', 'L2'] })
    )
  })
